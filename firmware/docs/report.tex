...
\title{
    \vspace{2cm}
    \textbf{\Huge BioPal Health Monitor Project Report}\\
    \vspace{1cm}
...
The demand for continuous, non-invasive health monitoring in wearable formats has grown exponentially. The BioPal Health Monitor bridges the gap between commercial off-the-shelf medical devices and open-source hobbyist electronics. By leveraging the ESP32's dual-core architecture and FreeRTOS, the device achieves deterministic real-time data acquisition without compromising network throughput or user interface responsiveness. This project places heavy emphasis on robust power management, precise task scheduling, and stable I2C bus arbitration.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Nodes
        \node[system] (esp32) {\textbf{BioPal}\\\textbf{ESP32 Monitor}};
...
\chapter{Conclusion}
The integration of a hardware-regulated power pipeline, strict I2C Mutexes, and prioritized FreeRTOS multitasking enables the BioPal Health Monitor to function with clinical-level reliability. The modular framework simplifies future expansions, such as integrating edge machine learning for advanced arrhythmia detection.
